...
\begin{fichacu}{CU-44}{Aplicar una sanción}
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      \item \textit{Sesion}: lee por token; cierra las del sancionado \textit{(pasos 7, 15)}
      \item \textit{Usuario}: lee el rol del moderador y del sancionado \textit{(pasos 7, 9)}
      \item \textit{Usuario}: escribe \texttt{estado\_\allowbreak{}cuenta}, solo si el ámbito es de cuenta \textit{(paso 14)}
      \item \textit{Sancion}: lee las activas del mismo ámbito \textit{(paso 10)}
      \item \textit{Sancion}: crea, y desactiva la anterior en el \mbox{FA-07} \textit{(pasos 13, \mbox{FA-07})}
      \item \textit{Reporte}: lee el de origen; escribe su resolución \textit{(pasos 1, 18)}
      \item \textit{Participacion}: cierra la que esté sin terminar, solo si el ámbito es de cuenta \textit{(paso 16)}
      \item \textit{BitacoraModeracion}: inserta \textit{(paso 20)}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
