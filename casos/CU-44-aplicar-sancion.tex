...
\begin{fichacu}{CU-44}{Aplicar una sanción}
  ...
  \textbf{Flujo normal} & \cuCelda{\begin{cuenum}[start=1]
      \item El SJM despliega la \texttt{GUI\_\allowbreak{}ApplySanction} con el jugador, el \textit{Reporte} que la origina, el historial de sanciones previas de ese jugador y los campos de ámbito, tipo y duración (\mbox{RN-06}). \textit{(Ver \mbox{FA-09})}
   ...
   & \cuCelda{\begin{cuenum}[start=9]
      ...
      \item El Servidor de partidas crea la \textit{Sancion} con el sancionado, el moderador que la aplica, el ámbito, el tipo, el motivo, la \texttt{fecha\_\allowbreak{}inicio}, la \texttt{fecha\_\allowbreak{}fin} y el reporte de origen; sin \texttt{fecha\_\allowbreak{}retiro}, nace \texttt{activa}. \textit{(Ver \mbox{EX-01})}
   ...
   & \cuCelda{\begin{cuenum}[start=17]
      ...
      \item El Servidor de partidas notifica al sancionado, si tiene sesión abierta, con \texttt{SANCTION\_\allowbreak{}APPLIED}, el motivo, el ámbito y la fecha de término. \textit{(Ver \mbox{EX-05})}
  ...
  \textbf{Flujos alternos} & \cuCelda{\textbf{\mbox{FA-01}: El Moderador decide no sancionar}\par
   ...
   & \cuCelda{\textbf{\mbox{FA-07}: Ya existe una sanción activa del mismo ámbito}\par
    \begin{cuenum}[start=1]
      ...
      \item Si el Moderador elige sustituir, el Servidor de partidas escribe en la anterior su \texttt{fecha\_\allowbreak{}retiro} y una referencia a la nueva, con lo que deja de estar \texttt{activa}, y el flujo continúa en el paso 12 del FN (\mbox{RN-09}).
  ...
  \textbf{Observación} & \cuCelda{\textit{Sobre por qué la sanción caduca sola y no hay caso que la levante.}\par
    Una sanción temporal no necesita un caso de uso que la retire: deja de aplicarse en cuanto su \texttt{fecha\_\allowbreak{}fin} queda atrás, y todos los flujos que la consultan —el inicio de sesión, el emparejamiento y el chat— comparan esa fecha con la actual. El único rastro que deja en la cuenta, el \texttt{estado\_\allowbreak{}cuenta} en \texttt{SUSPENDIDA}, lo devuelve a \texttt{ACTIVA} el propio inicio de sesión cuando la fecha ya pasó (\mbox{FA-19} del \mbox{CU-01}). Escribir un proceso que recorriera las sanciones vencidas para desactivarlas añadiría un trabajo periódico cuyo único efecto sería cambiar una bandera que nadie lee sin mirar también la fecha. \texttt{activa} existe para el \mbox{RN-09}, es decir, para distinguir una sanción retirada a mano de una que simplemente venció, no para saber si está surtiendo efecto ahora; por eso no se escribe, sino que se calcula a partir de \texttt{fecha\_\allowbreak{}retiro}.} \\ \hline
