...
\begin{fichacu}{CU-18}{Crear una sala privada}
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      ...
      \item \textit{Participacion}, \textit{ColaEmparejamiento}, \textit{SalaParticipante}, \textit{Sancion}: lee, para las precondiciones \textit{(paso 6)}
      \item \textit{Modo}: lee \textit{(paso 7)}
      \item \textit{Sala}: lee códigos en uso; crea; actualiza ajustes y \texttt{estado} \textit{(pasos 8, 9, 18, \mbox{FA-05}, \mbox{FA-06})}
      \item \textit{SalaParticipante}: crea, actualiza \texttt{listo}, elimina \textit{(pasos 10, 13, \mbox{FA-05} a \mbox{FA-07})}
      \item \textit{Partida}: crea con \texttt{tipo} \texttt{PRIVADA} \textit{(paso 17)}
      \item \textit{Participacion}, \textit{ParticipacionObjeto}: crea una por miembro \textit{(paso 17)}
      \item \textit{EstadisticaModo}, \textit{Equipamiento}: lee el elo de entrada y el aspecto de cada miembro \textit{(paso 17)}
    \end{cudatos}} \\
   & \cuCelda{\begin{cudatos}
      \item \textit{Mensaje}: crea los del chat de sala, con canal \texttt{SALA} \textit{(paso 12)}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
