...
\begin{fichacu}{CU-24}{Abandonar la partida}
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      \item \textit{Sesion}, \textit{Participacion}: lee por token \textit{(paso 5)}
      \item \textit{Partida}, \textit{Modo}: lee \texttt{estado}, \texttt{tipo} y número de jugadores \textit{(pasos 6, 9)}
      \item \textit{OfertaTablas}: marca las pendientes como \texttt{CADUCADA} \textit{(paso 7)}
      \item \textit{EstadisticaModo}: incrementa \texttt{abandonos} \textit{(paso 8)}
      \item \textit{Participacion}: lee \texttt{conectado} y \texttt{fecha\_\allowbreak{}desconexion} del rival; escribe resultado y posición en el \mbox{FA-05} \textit{(pasos \mbox{FA-05}, \mbox{FA-06})}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
