...
\begin{fichacu}{CU-17}{Elegir modo y buscar partida clasificatoria}
  ...
  \textbf{Flujo normal} & \cuCelda{\begin{cuenum}[start=1]
      ...
      \item El Servidor de partidas inserta al \textit{Usuario} en la \textit{ColaEmparejamiento} del modo y reloj elegidos, con su elo y la fecha de entrada. \textit{(Ver \mbox{EX-01})}
    \end{cuenum}} \\
   ...
   & \cuCelda{\begin{cuenum}[start=17]
      ...
      \item El Servidor de partidas notifica a cada cliente con \texttt{MATCH\_\allowbreak{}FOUND}, incluyendo el identificador de la partida, los participantes con su \texttt{nickname}, su elo y su equipamiento, y el orden de turno.
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      \item \textit{Sesion}: lee por token \textit{(paso 4)}
      \item \textit{Participacion}: lee, para comprobar que no haya ninguna sin terminar \textit{(paso 5)}
      \item \textit{Sancion}: lee las activas de ámbito de cuenta \textit{(paso 6)}
      \item \textit{EstadisticaModo}: lee \texttt{puntos\_\allowbreak{}elo} del modo elegido \textit{(paso 7)}
      \item \textit{Modo}: lee el catálogo y los parámetros del modo \textit{(pasos 1, 15)}
      \item \textit{ColaEmparejamiento}: inserta, consulta y borra \textit{(pasos 8, 11, 17)}
      \item \textit{Bloqueo}: lee, para descartar emparejamientos \textit{(paso 12)}
      \item \textit{Partida}: crea \textit{(paso 15)}
    \end{cudatos}} \\
   & \cuCelda{\begin{cudatos}
      \item \textit{Participacion}: crea una por jugador \textit{(paso 16)}
      \item \textit{Equipamiento}: lee, para fijar el aspecto de cada participante \textit{(paso 16)}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
