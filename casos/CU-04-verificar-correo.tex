...
\begin{fichacu}{CU-04}{Verificar el correo y activar la cuenta}
  ...
  \textbf{Flujo normal} & \cuCelda{\begin{cuenum}[start=1]
      \item El Jugador abre el enlace de verificación desde su cliente de correo. \textit{(Ver \mbox{FA-06})}
      \item El Servidor de partidas recibe la solicitud con el token incluido en el enlace. \textit{(Ver \mbox{EX-03})}
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      \item \textit{TokenVerificacionCorreo}: lee \texttt{token\_\allowbreak{}hash}, \texttt{usado}, \texttt{fecha\_\allowbreak{}expiracion} \textit{(pasos 3, 5, 6)}
      \item \textit{TokenVerificacionCorreo}: marca como usado e invalida los demás \textit{(pasos 10, 11)}
      \item \textit{Usuario}: lee \texttt{estado\_\allowbreak{}cuenta}, \texttt{idioma\_\allowbreak{}preferido} \textit{(pasos 7, 14)}
      \item \textit{Usuario}: escribe \texttt{estado\_\allowbreak{}cuenta}, \texttt{fecha\_\allowbreak{}verificacion} \textit{(paso 9)}
      \item \textit{Sancion}: lee, para el \mbox{FA-04} \textit{(paso 7)}
      \item \textit{BitacoraAcceso}: inserta \textit{(paso 13)}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
