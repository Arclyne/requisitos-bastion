...
\begin{fichacu}{CU-33}{Silenciar o bloquear a un jugador}
  ...
  \textbf{Flujo normal} & \cuCelda{\begin{cuenum}[start=1]
      ...
      \item El Servidor de partidas inicia una transacción y crea el \textit{Bloqueo} con el \texttt{id\_\allowbreak{}bloqueador}, el \texttt{id\_\allowbreak{}bloqueado} y la \texttt{fecha\_\allowbreak{}bloqueo}. \textit{(Ver \mbox{EX-01}, \mbox{FA-08})}
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      \item \textit{Sesion}, \textit{Usuario}: lee \textit{(paso 5)}
      \item \textit{Bloqueo}: lee; crea; elimina \textit{(pasos 6, 7, \mbox{FA-07})}
      \item \textit{Silencio}: crea; elimina \textit{(pasos \mbox{FA-02}, \mbox{FA-07})}
      \item \textit{Amistad}, \textit{Solicitud}: elimina las del par \textit{(paso 8)}
      \item \textit{Invitacion}: marca como \texttt{EXPIRADA} las pendientes del par \textit{(paso 8)}
      \item \textit{ColaEmparejamiento}, \textit{SalaParticipante}: lee; elimina la del retirado de la sala \textit{(paso 9)}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
