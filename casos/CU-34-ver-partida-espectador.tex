...
\begin{fichacu}{CU-34}{Ver una partida como espectador}
  ...
  \textbf{Flujo normal} & \cuCelda{\begin{cuenum}[start=1]
   ...
   & \cuCelda{\begin{cuenum}[start=9]
      \item El SJM despliega la \texttt{GUI\_\allowbreak{}Spectator} con el tablero en cámara libre, los jugadores, sus relojes, el historial de jugadas, el indicador de retraso, el número de espectadores y el chat de espectadores. \textit{(Ver \mbox{FA-07}, \mbox{FA-08}, \mbox{FA-10})}
  ...
  \textbf{Flujos alternos} & \cuCelda{\textbf{\mbox{FA-01}: El espectador entra desde el ranking o desde un perfil}\par
   ...
   & \cuCelda{\textbf{\mbox{FA-02}: El espectador abre un enlace compartido}\par
    \begin{cuenum}[start=1]
      ...
      \item El Servidor de partidas deriva el resumen del token y busca el \textit{EnlaceEspectador} con ese \texttt{token\_\allowbreak{}hash} cuya \textit{Partida} siga \texttt{EN\_\allowbreak{}CURSO}.
   ...
   & \cuCelda{\textbf{\mbox{FA-09}: Un jugador comparte su partida}\par
    \begin{cuenum}[start=1]
      ...
      \item El Servidor de partidas verifica que la partida admita espectadores y crea el \textit{EnlaceEspectador} con el \texttt{id\_\allowbreak{}partida}, el \texttt{id\_\allowbreak{}creador}, el resumen del token y la \texttt{fecha\_\allowbreak{}creacion}, o devuelve el existente.
      \item El SJM copia el enlace al portapapeles y lo indica (13e).
      \item Al finalizar la partida, el enlace deja de funcionar sin que nada se escriba en él, porque la partida ya no está \texttt{EN\_\allowbreak{}CURSO} (\mbox{RN-06}).
      \item Fin del caso de uso.
    \end{cuenum}} \\
  ...
  \textbf{Postcondiciones} & \cuCelda{\begin{cureglas}
      ...
      \item \textbf{\mbox{POST-03}} Si el caso termina por el \mbox{FA-09}, existe un \textit{EnlaceEspectador} para la partida, que dejará de funcionar al terminar.
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      \item \textbf{\textit{Sesion}:} lee por token, cuando hay sesión \textit{(paso 2)}
      \item \textbf{\textit{Partida}:} lee \texttt{estado}, \texttt{tipo} \textit{(paso 3)}
      \item \textbf{\textit{Participacion}, \textit{Usuario}:} lee los participantes y su \texttt{permite\_\allowbreak{}espectadores} \textit{(pasos 4, 7, 8)}
      \item \textbf{\textit{Sala}:} lee \texttt{permite\_\allowbreak{}espectadores}, si la partida viene de una sala \textit{(paso 4)}
      \item \textbf{\textit{Bloqueo}:} lee entre el espectador y los participantes \textit{(paso 5)}
      \item \textbf{\textit{Jugada}, \textit{ParticipacionObjeto}:} lee \textit{(paso 8)}
      \item \textbf{\textit{EnlaceEspectador}:} lee por \texttt{token\_\allowbreak{}hash}, con el estado de su \textit{Partida}; crea \textit{(pasos \mbox{FA-02}, \mbox{FA-09})}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
