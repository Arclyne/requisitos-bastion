...
\begin{fichacu}{CU-11}{Borrar la cuenta}
  ...
  \crudFila{Objetivo}{Permitir al Jugador dar de baja su cuenta con una confirmación en dos pasos, anonimizando sus datos personales en el mismo acto y sin romper el historial de las partidas que compartió con otros jugadores.}
  ...
  \textbf{Flujo normal} & \cuCelda{\begin{cuenum}[start=1]
      \item El Jugador selecciona ``borrar cuenta'' en la \texttt{GUI\_\allowbreak{}AccountSettings}. \textit{(Ver \mbox{FA-07})}
      \item El SJM despliega el primer paso de la \texttt{GUI\_\allowbreak{}DeleteAccount}: una lista de lo que se pierde —elo, monedas, objetos, amigos y posición en el ranking—, el aviso de que la baja es definitiva y no puede deshacerse, y un campo para la contraseña actual, con las opciones ``Continuar'' y ``Cancelar''. \textit{(Ver \mbox{FA-01})}
   ...
   & \cuCelda{\begin{cuenum}[start=9]
      ...
      \item El Servidor de partidas cambia el \texttt{estado\_\allowbreak{}cuenta} del \textit{Usuario} a \texttt{ELIMINADA}, escribe \texttt{fecha\_\allowbreak{}baja} con la fecha y hora actuales y anonimiza la cuenta: sustituye \texttt{nickname} y \texttt{correo} por valores anónimos únicos y deja \texttt{contrasena\_\allowbreak{}hash} y \texttt{contrasena\_\allowbreak{}sal} en nulo (\mbox{RN-02}, \mbox{RN-03}). \textit{(Ver \mbox{EX-01})}
      \item El Servidor de partidas elimina los \textit{EnlaceRed}, el \textit{HistorialNickname}, las \textit{Amistad}, los \textit{Silencio} y los \textit{Bloqueo} del \textit{Usuario}, e invalida su \textit{Avatar} (\mbox{RN-03}).
   ...
   & \cuCelda{\begin{cuenum}[start=17]
      ...
      \item El Servidor de partidas solicita al Servicio de correo un aviso de baja al correo que la cuenta tenía antes de anonimizarse, fuera de la transacción. \textit{(Ver \mbox{EX-06})}
      \item El Servidor de partidas responde con \texttt{DELETE\_\allowbreak{}ACCOUNT\_\allowbreak{}OK}.
      \item El SJM descarta el token y todos los datos del perfil guardados en memoria, y despliega la \texttt{GUI\_\allowbreak{}Login} con un aviso de que la cuenta se eliminó.
  ...
  \textbf{Excepciones} & \cuCelda{\textbf{\mbox{EX-01}: Fallo al escribir en la base de datos}\par
   ...
   & \cuCelda{\textbf{\mbox{EX-02}: Fallo al confirmar la transacción}\par
    \begin{cuenum}[start=1]
      ...
      \item El SJM muestra la \texttt{GUI\_\allowbreak{}MessageError} indicando que no fue posible confirmar la baja y que, si al volver a entrar sus credenciales ya no existen, la baja sí se aplicó.
      \item Fin del caso de uso.
    \end{cuenum}} \\
   ...
   & \cuCelda{\textbf{\mbox{EX-06}: El servicio de correo no está disponible}\par
    \begin{cuenum}[start=1]
      ...
      \item El flujo continúa en el paso 22 del FN.
    \end{cuenum}} \\ \hline
  \textbf{Postcondiciones} & \cuCelda{\begin{cureglas}
      \item \textbf{\mbox{POST-01}} Si el caso termina por el FN, el \textit{Usuario} tiene \texttt{estado\_\allowbreak{}cuenta} en \texttt{ELIMINADA}, \texttt{fecha\_\allowbreak{}baja} con la fecha de la baja, \texttt{nickname} y \texttt{correo} anónimos y contraseña en nulo.
      ...
      \item \textbf{\mbox{POST-05}} Si el caso termina por el FN, el \texttt{nickname} y el \texttt{correo} quedan libres para otro registro y la cuenta no puede recuperarse: ningún flujo devuelve una cuenta \texttt{ELIMINADA} a \texttt{ACTIVA}.
      \item \textbf{\mbox{POST-06}} Ninguna fila de \textit{Usuario} se elimina físicamente en ningún flujo.
    \end{cureglas}} \\ \hline
  \textbf{Reglas de negocio} & \cuCelda{\begin{cureglas}
      ...
      \item \textbf{\mbox{RN-02}} El borrado es lógico y definitivo: la cuenta pasa a \texttt{ELIMINADA} con \texttt{fecha\_\allowbreak{}baja} y no existe ningún periodo de recuperación (\mbox{D-07}).
      \item \textbf{\mbox{RN-03}} La cuenta se anonimiza en el mismo acto de la baja en lugar de eliminarse: los datos personales desaparecen y los identificadores quedan libres, pero la fila se conserva porque otros registros dependen de ella (\mbox{D-17}).
    ...
    \end{cureglas}} \\ \hline
  \textbf{Observación} & \cuCelda{\textit{Sobre el periodo de recuperación descartado.}\par
    \texttt{descripcion.md} preveía que la cuenta fuera recuperable durante catorce días, y el prototipo 9f dice, bajo el botón de borrar cuenta, que ``no hay vuelta atrás''. El equipo decidió prescindir del periodo de recuperación (\mbox{D-07}): un plazo de gracia obligaba a un flujo de recuperación en el inicio de sesión, a un proceso programado que anonimizara las cuentas vencidas y a mantener ocupados el \texttt{nickname} y el \texttt{correo} mientras tanto, a cambio de cubrir un arrepentimiento que la confirmación en dos pasos ya previene. Con la baja definitiva, el texto del prototipo 9f es correcto y la anonimización ocurre en la misma transacción que la baja.} \\ \hline
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      \item \textit{Sesion}: lee por token; cierra todas \textit{(pasos 8, 18)}
      \item \textit{Usuario}: lee \texttt{contrasena\_\allowbreak{}hash}, \texttt{contrasena\_\allowbreak{}sal}, \texttt{rol} \textit{(pasos 9, 11)}
      \item \textit{Usuario}: escribe \texttt{estado\_\allowbreak{}cuenta}, \texttt{fecha\_\allowbreak{}baja}; anonimiza \texttt{nickname}, \texttt{correo}, \texttt{contrasena\_\allowbreak{}hash}, \texttt{contrasena\_\allowbreak{}sal} \textit{(paso 13)}
      \item \textit{Participacion}: lee, para comprobar que no haya partida en curso \textit{(paso 10)}
      \item \textit{ColaEmparejamiento}, \textit{SalaParticipante}: elimina la del usuario \textit{(paso 15)}
      \item \textit{Solicitud}: elimina las pendientes \textit{(paso 16)}
      \item \textit{Invitacion}: marca como expiradas las pendientes \textit{(paso 16)}
      \item \textit{Reporte}: devuelve a \texttt{PENDIENTE} los que tenía en revisión \textit{(paso 17)}
    \end{cudatos}} \\
   & \cuCelda{\begin{cudatos}
      \item \textit{EnlaceRed}, \textit{HistorialNickname}, \textit{Amistad}, \textit{Silencio}, \textit{Bloqueo}: elimina \textit{(paso 14)}
      \item \textit{Avatar}: invalida \textit{(paso 14)}
      \item \textit{BitacoraAcceso}: inserta \textit{(paso 20)}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
