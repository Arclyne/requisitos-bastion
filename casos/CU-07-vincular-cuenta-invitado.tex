...
\begin{fichacu}{CU-07}{Vincular la cuenta de invitado}
  ...
  \textbf{Flujo normal} & \cuCelda{\begin{cuenum}[start=1]
      ...
      \item El SJM envía el mensaje \texttt{LINK\_\allowbreak{}ACCOUNT\_\allowbreak{}REQUEST} con los datos capturados, la contraseña, la versión de los términos aceptada y el token de la sesión de invitado. \textit{(Ver \mbox{EX-05}, \mbox{EX-06}, \mbox{EX-02})}
   ...
   & \cuCelda{\begin{cuenum}[start=9]
      \item El Servidor de partidas vuelve a aplicar sobre los datos recibidos todas las validaciones de los pasos 3 a 6, sin dar por buena la validación del cliente (\mbox{RN-09}). \textit{(Ver \mbox{FA-07})}
   ...
   & \cuCelda{\begin{cuenum}[start=17]
      \item El Servidor de partidas registra la \textit{AceptacionTerminos} con la versión aceptada, la fecha y la dirección de red de origen. \textit{(Ver \mbox{EX-03})}
  ...
  \textbf{Postcondiciones} & \cuCelda{\begin{cureglas}
      ...
      \item \textbf{\mbox{POST-03}} Si el caso termina por el FN, existe un registro de \textit{AceptacionTerminos} con la versión aceptada, su fecha y la dirección de red de origen.
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      \item \textit{Sesion}: lee por token, comprueba que sea de invitado \textit{(paso 8)}
      \item \textit{Sesion}: cierra con \texttt{motivo\_\allowbreak{}cierre} \textit{(paso 23)}
      \item \textit{Usuario}: lee \texttt{nickname} y \texttt{correo} de otros, para comprobar unicidad \textit{(pasos 11, 12)}
      \item \textit{Usuario}: actualiza \texttt{tipo\_\allowbreak{}cuenta}, \texttt{estado\_\allowbreak{}cuenta}, \texttt{nickname}, \texttt{correo}, \texttt{contrasena\_\allowbreak{}hash}, \texttt{contrasena\_\allowbreak{}sal}, \texttt{fecha\_\allowbreak{}nacimiento}, \texttt{codigo\_\allowbreak{}amigo}, \texttt{fecha\_\allowbreak{}ultimo\_\allowbreak{}envio\_\allowbreak{}verificacion} \textit{(pasos 16, 21)}
      \item \textit{PalabraProhibida}: lee el catálogo con ámbito de nickname \textit{(paso 10)}
      \item \textit{AceptacionTerminos}: crea \textit{(paso 17)}
      \item \textit{TokenVerificacionCorreo}: crea \textit{(paso 18)}
      \item \textit{BitacoraAcceso}: inserta \textit{(paso 22)}
    \end{cudatos}} \\
   & \cuCelda{\begin{cudatos}
      \item \textit{EstadisticaModo}, \textit{UsuarioObjeto}, \textit{Equipamiento}, \textit{Participacion}: \textbf{no se tocan}: conservarlas intactas es el objetivo del caso
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
