...
\begin{fichacu}{CU-45}{Apelar una sanción}
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      \item \textit{Sesion}: lee por token, cuando hay sesión \textit{(paso 6)}
      \item \textit{Sancion}: lee la del jugador \textit{(pasos 2, 7)}
      \item \textit{Apelacion}: lee la existente; crea \textit{(pasos 8, 10, \mbox{FA-01}, \mbox{FA-06})}
      \item \textit{PalabraProhibida}: lee, para marcar el texto \textit{(paso 9)}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
