...
\begin{fichacu}{CU-48}{Consultar las bitácoras}
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      \item \textbf{\textit{Sesion}, \textit{Usuario}:} lee el \texttt{rol} del administrador \textit{(paso 6)}
      \item \textbf{\textit{BitacoraAcceso}:} lee con filtros, agrupa por \texttt{direccion\_\allowbreak{}ip} \textit{(pasos 8, 9, \mbox{FA-05})}
      \item \textbf{\textit{BitacoraModeracion}:} lee con filtros \textit{(paso \mbox{FA-01})}
      \item \textbf{\textit{BitacoraModeracion}:} inserta la propia consulta \textit{(pasos 7, \mbox{FA-03})}
      \item \textbf{\textit{Usuario}:} lee el \texttt{nickname} de los usuarios de cada registro \textit{(pasos 8, \mbox{FA-01})}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
