...
\begin{fichacu}{CU-01}{Iniciar sesión}
  ...
  \textbf{Flujo normal} & \cuCelda{\begin{cuenum}[start=1]
   ...
   & \cuCelda{\begin{cuenum}[start=9]
      ...
      \item El Servidor de partidas verifica que el \texttt{estado\_\allowbreak{}cuenta} del \textit{Usuario} sea \texttt{ACTIVA}. \textit{(Ver \mbox{FA-09}, \mbox{FA-10}, \mbox{FA-17}, \mbox{FA-20})}
      ...
      \item El Servidor de partidas actualiza \texttt{fecha\_\allowbreak{}ultimo\_\allowbreak{}acceso} del \textit{Usuario}.
      \item El Servidor de partidas registra el acceso exitoso en la \textit{BitacoraAcceso} con el resultado \texttt{EXITO}.
    \end{cuenum}} \\
   & \cuCelda{\begin{cuenum}[start=17]
      ...
      \item El SJM aplica el \texttt{idioma\_\allowbreak{}preferido} recibido a los textos de la interfaz. \textit{(Ver \mbox{FA-18}, \mbox{FA-19})}
  ...
  \textbf{Flujos alternos} & \cuCelda{\textbf{\mbox{FA-01}: Salir del juego}\par
   ...
   & \cuCelda{\textbf{\mbox{FA-16}: El código del segundo factor es incorrecto}\par
    ...
    \end{cuenum}} \\
   & \cuCelda{\textbf{\mbox{FA-17}: La cuenta fue eliminada y sigue dentro del periodo de recuperación}\par
    \begin{cuenum}[start=1]
      \item El Servidor de partidas comprueba que no hayan transcurrido catorce días desde \texttt{fecha\_\allowbreak{}baja} (\mbox{RN-15}). Si ya transcurrieron, trata la cuenta como inexistente y el flujo continúa en el \mbox{FA-06}.
      \item El Servidor de partidas responde con \texttt{ACCOUNT\_\allowbreak{}DELETED\_\allowbreak{}RECOVERABLE} y los días que faltan para que la eliminación sea definitiva.
      \item El SJM despliega la \texttt{GUI\_\allowbreak{}MessageConfirm} indicando que la cuenta está programada para eliminarse, con las opciones ``Recuperar mi cuenta'' y ``Cancelar''.
      \item El Jugador selecciona ``Recuperar mi cuenta'' y el SJM envía el mensaje \texttt{RECOVER\_\allowbreak{}ACCOUNT\_\allowbreak{}REQUEST}. Si selecciona ``Cancelar'', fin del caso de uso.
      \item El Servidor de partidas cambia \texttt{estado\_\allowbreak{}cuenta} a \texttt{ACTIVA}, deja \texttt{fecha\_\allowbreak{}baja} en nulo y registra la recuperación en la \textit{BitacoraAcceso} con el resultado \texttt{CUENTA\_\allowbreak{}RECUPERADA}. \textit{(Ver \mbox{EX-04})}
      \item El flujo continúa en el paso 12 del FN.
    \end{cuenum}} \\
   & \cuCelda{\textbf{\mbox{FA-18}: Es el primer acceso de la cuenta}\par
    \begin{cuenum}[start=1]
      ...
      \item Fin del caso de uso.
    \end{cuenum}} \\
   & \cuCelda{\textbf{\mbox{FA-19}: El Jugador tiene una partida sin terminar}\par
    \begin{cuenum}[start=1]
      ...
      \item Fin del caso de uso.
    \end{cuenum}} \\
   & \cuCelda{\textbf{\mbox{FA-20}: La suspensión temporal de la cuenta ya venció}\par
    \begin{cuenum}[start=1]
      ...
      \item El Servidor de partidas devuelve el \texttt{estado\_\allowbreak{}cuenta} a \texttt{ACTIVA} y registra el cambio en la \textit{BitacoraAcceso} con el resultado \texttt{SUSPENSION\_\allowbreak{}VENCIDA} (\mbox{RN-17}). \textit{(Ver \mbox{EX-04})}
  ...
  \textbf{Excepciones} & \cuCelda{\textbf{\mbox{EX-01}: No se puede establecer la conexión con el servidor}\par
   ...
   & \cuCelda{\textbf{\mbox{EX-05}: Fallo al registrar la sesión}\par
    \begin{cuenum}[start=1]
      \item El Servidor de partidas revierte la transacción que abarca el registro de la \textit{Sesion} y la actualización de \texttt{fecha\_\allowbreak{}ultimo\_\allowbreak{}acceso}, de modo que no quede un acceso registrado sin sesión.
  ...
  \textbf{Postcondiciones} & \cuCelda{\begin{cureglas}
      ...
      \item \textbf{\mbox{POST-03}} Si el caso termina por el FN, \texttt{fecha\_\allowbreak{}ultimo\_\allowbreak{}acceso} del \textit{Usuario} contiene la fecha y hora del acceso.
   ...
   & \cuCelda{\begin{cureglas}
      \item \textbf{\mbox{POST-08}} Si el caso termina por el \mbox{FA-17} con la recuperación aceptada, el \textit{Usuario} vuelve a tener \texttt{estado\_\allowbreak{}cuenta} en \texttt{ACTIVA} y \texttt{fecha\_\allowbreak{}baja} nula.
    \end{cureglas}} \\ \hline
  \textbf{Reglas de negocio} & \cuCelda{\begin{cureglas}
      ...
      \item \textbf{\mbox{RN-04}} Solo puede iniciar sesión un \textit{Usuario} cuyo \texttt{estado\_\allowbreak{}cuenta} sea \texttt{ACTIVA}. Los valores \texttt{PENDIENTE}, \texttt{SUSPENDIDA}, \texttt{BANEADA} y \texttt{ELIMINADA} impiden el acceso, cada uno con su propio mensaje.
   ...
   & \cuCelda{\begin{cureglas}
    ...
    \end{cureglas}} \\
   & \cuCelda{\begin{cureglas}
      \item \textbf{\mbox{RN-15}} Una cuenta eliminada puede recuperarse iniciando sesión dentro de los catorce días siguientes a su \texttt{fecha\_\allowbreak{}baja}. Pasado ese plazo, la eliminación es definitiva (\mbox{D-07}).
      \item \textbf{\mbox{RN-16}} Las cuentas de invitado no inician sesión por este caso: no tienen \texttt{correo} ni contraseña y se recuperan con el token guardado en el dispositivo (\mbox{CU-06}).
          \item \textbf{\mbox{RN-17}} Una sanción temporal de cuenta no necesita un proceso que la levante: el \texttt{estado\_\allowbreak{}cuenta} vuelve a \texttt{ACTIVA} en el primer inicio de sesión posterior a su \texttt{fecha\_\allowbreak{}fin} (\mbox{FA-20}). El valor \texttt{SUSPENDIDA} indica que hubo una suspensión, pero la vigencia la decide siempre la \textit{Sancion}.
    \end{cureglas}} \\ \hline
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      \item \textit{Usuario}: lee \texttt{nickname}, \texttt{correo}, \texttt{tipo\_\allowbreak{}cuenta}, \texttt{contrasena\_\allowbreak{}hash}, \texttt{contrasena\_\allowbreak{}sal}, \texttt{bloqueada\_\allowbreak{}hasta}, \texttt{estado\_\allowbreak{}cuenta}, \texttt{fecha\_\allowbreak{}baja}, \texttt{doble\_\allowbreak{}factor\_\allowbreak{}habilitado}, \texttt{idioma\_\allowbreak{}preferido}, \texttt{id\_\allowbreak{}icono}, \texttt{fecha\_\allowbreak{}configuracion\_\allowbreak{}inicial} \textit{(pasos 6, 7, 8, 9, 11, 13, 17)}
      \item \textit{Usuario}: escribe \texttt{intentos\_\allowbreak{}fallidos}, \texttt{fecha\_\allowbreak{}ultimo\_\allowbreak{}intento\_\allowbreak{}fallido}, \texttt{bloqueada\_\allowbreak{}hasta}, \texttt{fecha\_\allowbreak{}ultimo\_\allowbreak{}acceso}, \texttt{fecha\_\allowbreak{}ultimo\_\allowbreak{}envio\_\allowbreak{}verificacion}; \texttt{estado\_\allowbreak{}cuenta} y \texttt{fecha\_\allowbreak{}baja} en el \mbox{FA-17}; \texttt{estado\_\allowbreak{}cuenta} en el \mbox{FA-20} \textit{(pasos 10, 15, \mbox{FA-08}, \mbox{FA-09}, \mbox{FA-17})}
      \item \textit{Sancion}: lee las activas de ámbito \texttt{CUENTA} \textit{(pasos 12, \mbox{FA-10})}
      \item \textit{Sesion}: crea \textit{(paso 14)}
      \item \textit{CodigoSegundoFactor}: crea, marca como consumido o invalidado, incrementa \texttt{intentos} \textit{(pasos \mbox{FA-11}, \mbox{FA-13} a \mbox{FA-16})}
      \item \textit{TokenVerificacionCorreo}: invalida los pendientes y crea uno nuevo \textit{(paso \mbox{FA-09})}
      \item \textit{EstadisticaModo}, \textit{Equipamiento}, \textit{Participacion}: lee, para el mensaje \texttt{LOGIN\_\allowbreak{}OK} \textit{(paso 17)}
      \item \textit{BitacoraAcceso}: inserta \textit{(pasos 16, \mbox{FA-06} a \mbox{FA-10}, \mbox{FA-13}, \mbox{FA-16}, \mbox{FA-17})}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
