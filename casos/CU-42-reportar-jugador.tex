...
\begin{fichacu}{CU-42}{Reportar a un jugador}
  ...
  \textbf{Flujo normal} & \cuCelda{\begin{cuenum}[start=1]
      ...
      \item El Servidor de partidas verifica que el reportado exista, no sea el denunciante y no tenga \texttt{estado\_\allowbreak{}cuenta} en \texttt{ELIMINADA}; y, si se adjunta una partida, que denunciante y reportado tengan \textit{Participacion} en ella (\mbox{RN-09}). \textit{(Ver \mbox{FA-06})}
  ...
  \textbf{Reglas de negocio} & \cuCelda{\begin{cureglas}
      ...
      \item \textbf{\mbox{RN-03}} Un denunciante no puede reportar dos veces al mismo jugador por la misma partida. Sí puede hacerlo por partidas distintas. Un reporte sin partida se trata como duplicado si ya existe otro sin partida del mismo denunciante al mismo jugador en las últimas veinticuatro horas.
   ...
   & \cuCelda{\begin{cureglas}
      \item \textbf{\mbox{RN-08}} El catálogo de \textit{MotivoReporte} tiene cinco entradas y se mantiene por SQL, no desde la aplicación (\mbox{D-09}).
          \item \textbf{\mbox{RN-09}} Solo puede adjuntarse una partida en la que participaron el denunciante y el reportado. De otro modo el reporte adjuntaría pruebas ajenas a los hechos.
    \end{cureglas}} \\ \hline
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      ...
      \item \textit{Usuario}: lee el reportado y su estado \textit{(paso 7)}
      \item \textit{Participacion}: lee, para comprobar que ambos jugaron la partida adjunta \textit{(paso 7)}
      \item \textit{MotivoReporte}: lee el catálogo \textit{(pasos 2, 8)}
      \item \textit{Reporte}: lee, para comprobar duplicados y el límite \textit{(pasos 9, 10)}
      \item \textit{Reporte}: crea \textit{(pasos 12, 13)}
      \item \textit{Partida}: referencia, sin copiar nada de ella \textit{(paso 13)}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
