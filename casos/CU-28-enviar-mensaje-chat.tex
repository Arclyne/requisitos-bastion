...
\begin{fichacu}{CU-28}{Enviar un mensaje de chat}
  ...
  \textbf{Flujo normal} & \cuCelda{\begin{cuenum}[start=1]
      \item El Jugador abre el chat desde su icono y selecciona el canal: partida, global o amigos.
   ...
   & \cuCelda{\begin{cuenum}[start=9]
      ...
      \item El Servidor de partidas contrasta el texto contra el catálogo de \textit{PalabraProhibida} con ámbito de chat o de ambos, en el idioma del canal y en los que apliquen a todos (\mbox{RN-01}). \textit{(Ver \mbox{FA-07})}
      ...
      \item El Servidor de partidas entrega el mensaje a los destinatarios con \texttt{CHAT\_\allowbreak{}MESSAGE}, omitiendo a quienes hayan silenciado al autor (\mbox{RN-07}).
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      ...
      \item \textit{Sancion}: lee las activas de ámbito de chat \textit{(paso 7)}
      \item \textit{Participacion}, \textit{Amistad}: lee, para comprobar el acceso al canal \textit{(paso 8)}
      \item \textit{Mensaje}: lee los recientes del canal \textit{(paso 2)}
      \item \textit{Mensaje}: crea \textit{(paso 12)}
      \item \textit{PalabraProhibida}: lee el catálogo con ámbito de chat \textit{(paso 11)}
      \item \textit{Bloqueo}, \textit{Silencio}: lee, para decidir la entrega \textit{(pasos 13, 14)}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
