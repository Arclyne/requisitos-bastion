...
\begin{fichacu}{CU-26}{Reconectar a una partida en curso}
  ...
  \crudFila{Disparador}{El SJM recupera la conexión durante una partida, o recibe una \textit{Participacion} sin terminar al iniciar sesión (\mbox{FA-19} del \mbox{CU-01}), o el Jugador selecciona ``volver a la partida'' en un aviso de otro caso de uso.}
  ...
  \textbf{Flujos alternos} & \cuCelda{\textbf{\mbox{FA-01}: El Jugador no quiere volver todavía}\par
   ...
   & \cuCelda{\textbf{\mbox{FA-02}: La sesión ya no es válida}\par
    \begin{cuenum}[start=1]
      ...
      \item El SJM despliega la \texttt{GUI\_\allowbreak{}Login}. Al iniciar sesión de nuevo, el \mbox{FA-19} del \mbox{CU-01} lo traerá de vuelta a este caso.
      \item Fin del caso de uso.
    \end{cuenum}} \\
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      ...
      \item \textit{Participacion}: escribe \texttt{conectado} y \texttt{fecha\_\allowbreak{}desconexion} \textit{(pasos 1, 10, \mbox{FA-07})}
      \item \textit{Participacion}: lee las de todos los jugadores con casilla, muros, reloj y posición \textit{(pasos 7, 9)}
      \item \textit{Partida}: lee \texttt{estado}, \texttt{turno\_\allowbreak{}actual} \textit{(paso 7)}
      \item \textit{Jugada}: lee todas las de la partida en orden \textit{(paso 9)}
      \item \textit{ParticipacionObjeto}, \textit{OfertaTablas}: lee \textit{(paso 9)}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
