...
\begin{fichacu}{CU-25}{Finalizar la partida y aplicar la progresión}
  ...
  \textbf{Flujo normal} & \cuCelda{\begin{cuenum}[start=1]
      \item El Servidor de partidas detecta la condición de término y determina la forma de término: \texttt{META}, \texttt{TIEMPO\_\allowbreak{}AGOTADO}, \texttt{RENDICION}, \texttt{ABANDONO} o \texttt{TABLAS} (\mbox{RN-01}). \textit{(Ver \mbox{FA-04}, \mbox{FA-05}, \mbox{FA-06})}
      ...
      \item El Servidor de partidas actualiza la \textit{Partida}: estado \texttt{FINALIZADA}, \texttt{fecha\_\allowbreak{}fin}, forma de término y el ganador, si lo hay. \textit{(Ver \mbox{EX-01}, \mbox{FA-08})}
   ...
   & \cuCelda{\begin{cuenum}[start=9]
      ...
      \item El Servidor de partidas notifica a cada cliente con \texttt{MATCH\_\allowbreak{}ENDED}, incluyendo el resultado, la forma de término, el cambio de elo, la experiencia y las monedas obtenidas. \textit{(Ver \mbox{FA-07}, \mbox{EX-03})}
  ...
  \textbf{Flujos alternos} & \cuCelda{\textbf{\mbox{FA-01}: La partida no es clasificatoria}\par
    \begin{cuenum}[start=1]
      ...
      \item El Servidor de partidas no modifica la \textit{EstadisticaModo}: sus contadores describen solo partidas clasificatorias, para que cinco partidas privadas no basten para entrar en el ranking (\mbox{RN-14}). Otorga la experiencia del paso 11.
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      \item \textit{Partida}: escribe estado, \texttt{fecha\_\allowbreak{}fin}, forma de término, ganador \textit{(paso 5)}
      \item \textit{Participacion}: escribe resultado, posición, elo final, diferencia, reloj restante \textit{(pasos 6, 8, 13)}
      \item \textit{Jugada}: lee, para contar muros colocados y para el repaso de la IA \textit{(pasos 9, \mbox{FA-01})}
      \item \textit{EstadisticaModo}: escribe todos los contadores, \texttt{fecha\_\allowbreak{}ultima\_\allowbreak{}partida} y la división \textit{(pasos 9, \mbox{FA-02})}
      \item \textit{HistorialDivision}: crea uno por cambio de división \textit{(paso FA-02)}
      \item \textit{Usuario}: escribe experiencia, nivel y saldo de monedas \textit{(pasos 11, 12, \mbox{FA-03})}
      \item \textit{MovimientoMoneda}: crea uno por participante \textit{(pasos 12, \mbox{FA-03})}
      \item \textit{UsuarioObjeto}: crea, por las recompensas de nivel \textit{(paso \mbox{FA-03})}
      \item \textit{Caja}: crea las de recompensa \textit{(paso FA-03)}
      \item \textit{EnlaceEspectador}: desactiva los de la partida \textit{(paso FA-08)}
      \item \textit{Sala}, \textit{SalaParticipante}: cierra la sala de origen y elimina sus miembros \textit{(paso FA-08)}
      \item \textit{Modo}: lee, para los parámetros de puntuación \textit{(paso 7)}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
