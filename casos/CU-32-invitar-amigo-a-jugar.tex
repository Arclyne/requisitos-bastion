...
\begin{fichacu}{CU-32}{Invitar a un amigo a jugar}
  ...
  \textbf{Flujo normal} & \cuCelda{\begin{cuenum}[start=1]
   ...
   & \cuCelda{\begin{cuenum}[start=9]
      ...
      \item El Servidor de partidas marca la \textit{Invitacion} como \texttt{ACEPTADA} y agrega al invitado a la sala conforme a los pasos 9 a 11 del \mbox{CU-19}, sin pedir el código (\mbox{RN-05}).
    \end{cuenum}} \\
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      \item \textit{Sesion}: lee la del emisor y comprueba la del destinatario \textit{(pasos 5, 7)}
      \item \textit{Participacion}, \textit{ColaEmparejamiento}: lee, para las exclusiones \textit{(pasos 5, 7)}
      \item \textit{Bloqueo}, \textit{Usuario}: lee \textit{(paso 6)}
      \item \textit{Invitacion}: lee las pendientes; crea; escribe \texttt{estado} \textit{(pasos 8, 10, 16, \mbox{FA-07}, \mbox{FA-08}, \mbox{FA-09})}
      \item \textit{Sala}, \textit{SalaParticipante}: crea o usa la sala; agrega al invitado \textit{(pasos 9, 16)}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
