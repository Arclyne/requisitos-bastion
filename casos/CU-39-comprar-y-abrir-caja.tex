...
\begin{fichacu}{CU-39}{Comprar y abrir una caja}
  ...
  \textbf{Flujo normal} & \cuCelda{\begin{cuenum}[start=1]
   ...
   & \cuCelda{\begin{cuenum}[start=17]
      ...
      \item El Servidor de partidas sortea tres objetos del \textit{TipoCaja} conforme a su tabla de probabilidades, descartando los que el \textit{Usuario} ya posea por duplicado y aplicando la garantía de rareza si procede (\mbox{RN-03}, \mbox{RN-04}, \mbox{RN-05}). \textit{(Ver \mbox{EX-05}, \mbox{FA-10})}
      \item El Servidor de partidas inicia una transacción sobre la Base de datos.
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      ...
      \item \textbf{\textit{TipoCaja}:} lee precio, actividad y tabla de probabilidades \textit{(pasos 2, 8, 20)}
      \item \textbf{\textit{TipoCajaObjeto}:} lee los objetos posibles y su probabilidad \textit{(pasos 2, 20)}
      \item \textbf{\textit{Caja}:} crea, lee y marca como abierta o caducada \textit{(pasos 11, 18, 26)}
      \item \textbf{\textit{CajaContenido}:} crea tres \textit{(paso 24)}
      \item \textbf{\textit{ObjetoCosmetico}:} lee los candidatos del sorteo \textit{(paso 20)}
      \item \textbf{\textit{UsuarioObjeto}:} lee, para descartar repetidos; crea los nuevos \textit{(pasos 20, 22)}
      \item \textbf{\textit{Usuario}:} lee y escribe saldo y contador de cajas sin raro \textit{(pasos 9, 12, 23, 25)}
      \item \textbf{\textit{MovimientoMoneda}:} crea de tipo \texttt{COMPRA\_\allowbreak{}CAJA}, \texttt{CONVERSION} y, si procede, \texttt{DEVOLUCION} \textit{(pasos 12, 23)}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
