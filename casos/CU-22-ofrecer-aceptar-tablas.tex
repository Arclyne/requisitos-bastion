...
\begin{fichacu}{CU-22}{Ofrecer y aceptar tablas}
  ...
  \textbf{Flujo normal} & \cuCelda{\begin{cuenum}[start=1]
   ...
   & \cuCelda{\begin{cuenum}[start=9]
      ...
      \item El flujo continúa en el \mbox{CU-25} Finalizar la partida, con la forma de término \texttt{TABLAS}. \textit{(Ver \mbox{EX-02})}
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      \item \textit{Sesion}, \textit{Participacion}: lee por token \textit{(paso 3)}
      \item \textit{Partida}, \textit{Modo}: lee \texttt{estado}, \texttt{tipo} y número de jugadores \textit{(pasos 3, 11, \mbox{PRE-02})}
      \item \textit{OfertaTablas}: lee las pendientes y las recientes del jugador \textit{(pasos 4, 5)}
      \item \textit{OfertaTablas}: crea; escribe \texttt{estado} y \texttt{fecha\_\allowbreak{}respuesta} \textit{(pasos 6, 12, \mbox{FA-05}, \mbox{FA-06})}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
