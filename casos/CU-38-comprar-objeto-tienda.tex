...
\begin{fichacu}{CU-38}{Comprar un objeto en la tienda}
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      ...
      \item \textit{ObjetoCosmetico}: lee precio, disponibilidad y nivel requerido \textit{(pasos 9, 10, 12)}
      \item \textit{UsuarioObjeto}: lee, para comprobar que no lo posea \textit{(paso 11)}
      \item \textit{UsuarioObjeto}: crea \textit{(paso 15)}
      \item \textit{Usuario}: lee saldo y nivel; escribe saldo \textit{(pasos 12, 13, 17)}
      \item \textit{MovimientoMoneda}: crea \textit{(paso 16)}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
