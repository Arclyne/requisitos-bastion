...
\begin{fichacu}{CU-19}{Unirse a una sala con código}
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      ...
      \item \textit{Participacion}, \textit{ColaEmparejamiento}, \textit{SalaParticipante}, \textit{Sancion}: lee, para las precondiciones \textit{(paso 5)}
      \item \textit{Sala}: lee por \texttt{codigo} y \texttt{estado} \textit{(paso 6)}
      \item \textit{Bloqueo}: lee entre el jugador y los miembros \textit{(paso 7)}
      \item \textit{Modo}: lee el número de plazas \textit{(paso 8)}
      \item \textit{SalaParticipante}: crea, actualiza \texttt{listo}, elimina \textit{(pasos 9, 14, \mbox{FA-05}, \mbox{EX-03})}
      \item \textit{Mensaje}: lee los recientes del chat de sala \textit{(paso 11)}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
