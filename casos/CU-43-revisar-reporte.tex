...
\begin{fichacu}{CU-43}{Revisar un reporte y resolverlo}
  ...
  \textbf{Flujos alternos} & \cuCelda{\textbf{\mbox{FA-01}: La sesión no es válida o el usuario ya no es moderador}\par
   ...
   & \cuCelda{\textbf{\mbox{FA-08}: El Moderador toma una apelación}\par
    \begin{cuenum}[start=1]
      ...
      \item Si la retira, el Servidor de partidas marca la \textit{Apelacion} como \texttt{ACEPTADA} y, en la misma transacción, pone la \textit{Sancion} con \texttt{activa} en falso y \texttt{fecha\_\allowbreak{}retiro}; si era de ámbito de cuenta, devuelve el \texttt{estado\_\allowbreak{}cuenta} a \texttt{ACTIVA} (\mbox{RN-06}).
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      \item \textit{Sesion}, \textit{Usuario}: lee por token y el \texttt{rol} \textit{(paso 3)}
      \item \textit{Reporte}: lee la cola; escribe \texttt{estado}, \texttt{id\_\allowbreak{}moderador}, \texttt{fecha\_\allowbreak{}asignacion}, \texttt{nota\_\allowbreak{}resolucion}, \texttt{fecha\_\allowbreak{}resolucion} \textit{(pasos 4, 5, 10, 17, \mbox{FA-06})}
      \item \textit{MotivoReporte}: lee \textit{(paso 5)}
      \item \textit{Apelacion}: lee la cola; escribe \texttt{estado}, \texttt{id\_\allowbreak{}moderador}, \texttt{nota\_\allowbreak{}resolucion}, \texttt{fecha\_\allowbreak{}resolucion} \textit{(pasos 5, \mbox{FA-08})}
      \item \textit{Partida}, \textit{Jugada}, \textit{Mensaje}: lee las pruebas \textit{(paso 11)}
      \item \textit{Sancion}: lee el historial; retira la apelada en el \mbox{FA-08} \textit{(pasos 11, \mbox{FA-08})}
      \item \textit{Usuario}: escribe \texttt{estado\_\allowbreak{}cuenta}, al retirar una sanción de cuenta \textit{(paso \mbox{FA-08})}
      \item \textit{BitacoraModeracion}: inserta \textit{(pasos 12, 18, \mbox{FA-06}, \mbox{FA-08})}
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
