...
\begin{fichacu}{CU-47}{Retirar a un moderador}
  ...
  \textbf{Flujo normal} & \cuCelda{\begin{cuenum}[start=1]
      ...
      \item El Servidor de partidas inicia una transacción y cambia el \texttt{rol} del afectado a \texttt{JUGADOR}. \textit{(Ver \mbox{EX-01})}
  ...
  \textbf{Datos que toca} & \cuCelda{\begin{cudatos}
      \item \textit{Sesion}, \textit{Usuario}: lee el \texttt{rol} del administrador y del afectado \textit{(pasos 5, 6)}
      \item \textit{Usuario}: escribe \texttt{rol} \textit{(paso 7)}
      \item \textit{Reporte}, \textit{Apelacion}: devuelve a \texttt{PENDIENTE} los que tenía en revisión \textit{(paso 8)}
      \item \textit{BitacoraModeracion}: inserta \textit{(pasos 9, \mbox{FA-02}, \mbox{FA-04})}
      \item \textit{Sancion}: no se toca
    \end{cudatos}} \\ \hline
\end{fichacu}
\clearpage
