...
\textbf{Caso} & \textbf{Qué cambia} \\ \hline
\endhead
\mbox{CU-01} & Reescrito: la \mbox{RN-06} pasa a permitir varias sesiones por cuenta y desaparece el flujo alterno de sesión ya abierta \\ \hline
...
\flujoTitulo{\mbox{D-02} a \mbox{D-21}. Decisiones adoptadas para poder redactar}
...
\mbox{D-07} & \textbf{Borrado lógico.} El documento de diseño del juego manda sobre el prototipo 9f: la cuenta pasa a \texttt{ELIMINADA} con \texttt{fecha\_\allowbreak{}baja}. Se descarta el periodo de recuperación de catorce días que preveía \texttt{descripcion.md}: la baja es definitiva, como dice el prototipo 9f. La fila de \texttt{Usuario} nunca se borra físicamente (ver \mbox{D-17}) \\ \hline
...
\mbox{D-17} & \textbf{Anonimización inmediata.} La cuenta se anonimiza en la misma transacción de la baja (\mbox{CU-11}): se liberan \texttt{nickname} y \texttt{correo}, la contraseña queda en nulo y se borran enlaces, historial de nombres, amistades, silencios y bloqueos, pero la fila se conserva. No hace falta un proceso programado ni un flujo de recuperación \\ \hline
...
\mbox{D-21} & \textbf{Internacionalización con diccionarios de recursos y texto en UTF-8.} Los idiomas admitidos son \texttt{es-MX}, por omisión, y \texttt{en}. Todo texto que se muestra se traduce con diccionarios de recursos: los del cliente para la interfaz, los términos de uso, los valores de dominio y los nombres de los catálogos, y los del servidor de partidas para los correos. Si a un diccionario le falta una clave, se usa la de \texttt{es-MX}. La base de datos no guarda traducciones: cada fila de catálogo guarda un \texttt{codigo}, como \texttt{CLASICO} o \texttt{PEON\_\allowbreak{}CLASICO}, que es la clave de su nombre en los diccionarios. Sí guarda el \texttt{idioma\_\allowbreak{}preferido} de cada cuenta, el idioma en que se aceptaron los términos y el de cada término prohibido, nulo si se filtra en todos, con un \texttt{CHECK} de los idiomas admitidos. La base se crea con la intercalación \texttt{Modern\_\allowbreak{}Spanish\_\allowbreak{}100\_\allowbreak{}CI\_\allowbreak{}AS\_\allowbreak{}SC\_\allowbreak{}UTF8}, para que todo texto admita la ñ, los acentos y cualquier carácter Unicode. Lo que escriben las personas no se traduce (\mbox{CU-28} \mbox{RN-12}) \\ \hline
\end{longtable}
\end{center}

\subsection*{Prototipos y casos de uso}
\addcontentsline{toc}{subsection}{Prototipos y casos de uso}
De las 125 pantallas del prototipo de interfaz, 57 ilustran una funcionalidad y 68 se descartaron. La lista completa, con el motivo de cada descarte y la pantalla que la sustituye, está en la sección \textit{Trazabilidad de prototipos} del análisis de funcionalidades CRUD. En resumen: las variantes de disposición de una misma pantalla, las iteraciones anteriores del HUD de partida, los catálogos de mensajes de error, los estados vacíos y de carga, y los ajustes que se guardan solo en el cliente no generan caso de uso ni funcionalidad.
